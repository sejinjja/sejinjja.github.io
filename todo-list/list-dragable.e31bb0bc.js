...
var list = Array(3).fill('not to do');
...
function initTodo(_ref) {
  var order = _ref.order,
      item = _ref.item;
  var itemElm = createElementFromHtml("<label class=\"todo\" style=\"--order: ".concat(order, "\" draggable=\"true\">\n                    <input type=\"checkbox\" class=\"check\">\n                    <div class=\"done\"></div>\n                    <div class=\"subject\">").concat(order, " : ").concat(item, "</div>\n              </label>"));
  ...
  addListenerMulti(itemElm, 'touchmove', function (event) {
    ...
    var x = touch.clientX,
        y = touch.clientY;
    var targetElm = Array.from(document.getElementsByClassName('todo')).find(function (todoElm) {
      return x >= todoElm.offsetLeft && x <= todoElm.offsetLeft + todoElm.offsetWidth && y >= todoElm.offsetTop && y <= todoElm.offsetTop + todoElm.offsetHeight;
    });
    swap(targetElm);
  });
...
document.getElementById('adder').addEventListener('keyup', function (_ref2) {
  var key = _ref2.key,
      target = _ref2.target;

  if (key === 'Enter') {
    var item = target.value.trim();
...
function swap(targetElm) {
  if (draggedElm && targetElm) {
    ...
    var tmpCssText = draggedElm.style.cssText;
...
//# sourceMappingURL=/list-dragable.e31bb0bc.js.map