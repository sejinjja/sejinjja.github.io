...
GitHub also requires the workflow file to exist on the default branch before `pull_request_target` events will trigger. Merge `.github/workflows/auto-merge.yml` into the default branch first, then run the verification PR flow below.
